...
4. **Polish:** ✅ multi-client orchestration (all cameras drain one queue toward a
   shared target) + ✅ visited history (`VisitedStore`) + ✅ hard negatives
   (surface confuser scan + `saveEmpty` capture flag → empty-label frames) + ✅
